...
class PGNN_layer(nn.Module):
    ...
    def forward(self, feature, dists_max, dists_argmax, dmx2=None, dagmx2=None):
        # if args.AdverserialAttack:
        #     if(torch.all(torch.eq(dists_max, dmx2))):
        #             print("dist max same")
        #     else:
        #         print(dists_max.shape, dmx2.shape)
        #     if(torch.all(torch.eq(dists_argmax, dagmx2))):
        #         print("dists arg max same")
        #     else:
        #         print(dists_argmax.shape, dagmx2.shape)
        if self.dist_trainable:
...
class PGNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        if self.feature_pre:
            x = self.linear_pre(x)
        if args.AdverserialAttack:
            x_position, x = self.conv_first(x, data.dists_max, data.dists_argmax, data.dists_max_advAttack, data.dists_argmaxadvAttack)
        else:
        ...
        for i in range(self.layer_num-2):
            ...
            if self.dropout:
                x = F.dropout(x, training=self.training)
        if args.AdverserialAttack:
            x_position, x = self.conv_out(x, data.dists_max, data.dists_argmax, data.dists_max_advAttack, data.dists_argmaxadvAttack)
        else:
