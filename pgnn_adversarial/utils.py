...
def get_link_mask(data, remove_ratio=0.2, resplit=True, infer_link_positive=True):
    if resplit:
        ...
        data.mask_link_positive_train, data.mask_link_positive_val, data.mask_link_positive_test = split_edges(data.mask_link_positive, remove_ratio)
...
def all_pairs_shortest_path_length_parallel(graph,cutoff=None,num_workers=1):
    ...
    #     dists_dict[node] = nx.single_source_shortest_path_length(graph, node)
...
def precompute_dist_data(edge_index, num_nodes, approximate=0):
        ...
        for i, node_i in enumerate(graph.nodes()):
            ...
            for j, node_j in enumerate(graph.nodes()):
                ...
                #print('node_i',node_i,'node_j',node_j,'dist',dist)
                if dist!=-1:
        ...
        # print('dists_dict')
        # for k, v in dists_dict.items():
        #     print(k,":", v)
        # #print('dists_array')
        # for arr in dists_array:
        #     print(list(arr))
        # exit()
        return dists_array
